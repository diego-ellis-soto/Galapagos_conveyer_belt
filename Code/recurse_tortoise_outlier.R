...
pond_uniques_sp_laea_df <- data.frame(pond_uniques_sp)|> dplyr::select(x,y,GPS_Pond_name)  |> dplyr::select(x,y)
...
ggplot(data = rec$revisitStats, aes(x = timeInside, fill = id)) + 
...
# Giant tortoises
boxplot(rec$revisitStats$timeInside ~ as.numeric(format(rec$revisitStats$entranceTime, "%H")),
...
hist(rec$revisitStats$timeSinceLastVisit,
     xlab = "Time since last visit (h)", main = "")


# rec = rec[rec$revisitStats$timeSinceLastVisit < 1000]

plot(rec$revisitStats$timeSinceLastVisit, rec$revisitStats$timeInside,
     xlab = "Time since last visit (h)", ylab = "Time inside (h)")
lines(lowess(x = rec$revisitStats$timeSinceLastVisit, y = rec$revisitStats$timeInside, delta = 0.01 * diff(range(rec$revisitStats$timeSinceLastVisit, na.rm = TRUE))), col = "red")
